...
S_IB = "tests/test_initial_balance_reversion.py"
S_IB_GOV = "tests/test_initial_balance_governor.py"
S_IB_CAUS = "tests/test_initial_balance_causality.py"
S_IB_EXEC = "tests/test_initial_balance_execution.py"
...
IBS = "quant_brain/strategies/initial_balance_reversion/spec.py"
IBI = "quant_brain/strategies/initial_balance_reversion/indicators.py"
IBE = "quant_brain/strategies/initial_balance_reversion/engine.py"
IBG = "quant_brain/strategies/initial_balance_reversion/governor.py"
...
MUTATIONS: list[tuple] = [
    ...
    # ---- ES V1.0_Frozen: Initial Balance Statistical Reversion ----------------
    # Contract math, session clock, IB construction, the excursion extreme the stop
    # depends on, causality, and every risk rule. Several of these are defects this
    # repository has actually shipped before.

    ('ib__commission_reverted_to_the_repository_rate', IBS,
     '    commission_round_turn: float = 4.14',
     '    commission_round_turn: float = 3.78',
     S_IB),
    ('ib__point_value_wrong_by_a_factor_of_ten', IBS,
     '    point_value_per_contract: float = 50.0',
     '    point_value_per_contract: float = 5.0',
     S_IB),
    ('ib__tick_size_doubled', IBS,
     '    tick: float = 0.25',
     '    tick: float = 0.50',
     S_IB),
    ('ib__ib_window_starts_an_hour_early', IBS,
     '    ib_start: tuple[int, int] = (9, 30)',
     '    ib_start: tuple[int, int] = (8, 30)',
     S_IB),
    ('ib__ib_window_ends_late', IBS,
     '    ib_end: tuple[int, int] = (10, 30)',
     '    ib_end: tuple[int, int] = (11, 0)',
     S_IB),
    ('ib__entry_cutoff_moved_past_1400', IBS,
     '    execution_cutoff: tuple[int, int] = (14, 0)',
     '    execution_cutoff: tuple[int, int] = (15, 0)',
     S_IB_GOV),
    ('ib__hard_flatten_moved', IBS,
     '    hard_flatten: tuple[int, int] = (15, 45)',
     '    hard_flatten: tuple[int, int] = (16, 0)',
     S_IB),
    ('ib__the_venue_clock_becomes_utc', IBS,
     'TIMEZONE: ZoneInfo = ET',
     'TIMEZONE: ZoneInfo = ZoneInfo("UTC")',
     S_IB),
    ('ib__regime_floor_becomes_exclusive', IBI,
     '        return lo <= self.ib_range <= hi',
     '        return lo < self.ib_range <= hi',
     S_IB),
    ('ib__regime_ceiling_becomes_exclusive', IBI,
     '        return lo <= self.ib_range <= hi',
     '        return lo <= self.ib_range < hi',
     S_IB),
    ('ib__regime_floor_shaved', IBS,
     '    ib_range_min: float = 10.0',
     '    ib_range_min: float = 9.0',
     S_IB),
    ('ib__regime_ceiling_raised', IBS,
     '    ib_range_max: float = 35.0',
     '    ib_range_max: float = 40.0',
     S_IB),
    ('ib__the_ib_keeps_absorbing_past_1030', IBI,
     '        if minute_of_start >= self.end_minute:\n            self.seal()\n            return',
     '        if minute_of_start >= self.end_minute + 30:\n            self.seal()\n            return',
     S_IB_CAUS),
    ('ib__the_ib_can_be_read_before_it_closes', IBI,
     '        if not self.complete:\n            raise IBNotReady(',
     '        if False:\n            raise IBNotReady(',
     S_IB),
    ('ib__an_empty_ib_window_is_sealed_silently', IBI,
     '        if self.buckets == 0:\n            raise IBNotReady(',
     '        if False:\n            raise IBNotReady(',
     S_IB),
    ('ib__midpoint_uses_the_high_instead_of_the_average', IBI,
     '        return (self._sealed_high + self._sealed_low) / 2.0',
     '        return self._sealed_high',
     S_IB),
    ('ib__range_sign_flipped', IBI,
     '        return self._sealed_high - self._sealed_low',
     '        return self._sealed_low - self._sealed_high',
     S_IB),
    ('ib__excursion_extreme_stops_deepening', IBI,
     '                self.extreme = min(self.extreme, five.low)',
     '                self.extreme = self.extreme',
     S_IB),
    ('ib__short_excursion_extreme_stops_deepening', IBI,
     '            self.extreme = max(self.extreme, five.high)',
     '            self.extreme = self.extreme',
     S_IB),
    ('ib__excursion_opens_on_the_close_instead_of_the_low', IBI,
     '                self.side, self.extreme, self.buckets, self.consumed = BELOW, five.low, 1, False',
     '                self.side, self.extreme, self.buckets, self.consumed = BELOW, five.close, 1, False',
     S_IB),
    ('ib__a_consumed_excursion_can_be_traded_twice', IBI,
     '        if self.side == NONE or self.consumed:',
     '        if self.side == NONE:',
     S_IB),
    ('ib__stop_sits_on_the_extreme_instead_of_one_tick_beyond', IBS,
     '    stop_offset_ticks: float = 1.0',
     '    stop_offset_ticks: float = 0.0',
     S_IB),
    ('ib__the_target_rounds_toward_the_entry', IBS,
     '        if direction > 0:\n            return round(math.ceil(midpoint / self.tick) * self.tick, 10)\n        return round(math.floor(midpoint / self.tick) * self.tick, 10)',
     '        if direction > 0:\n            return round(math.floor(midpoint / self.tick) * self.tick, 10)\n        return round(math.ceil(midpoint / self.tick) * self.tick, 10)',
     S_IB),
    ('ib__entry_slippage_helps_instead_of_hurting', IBE,
     '        fill = self.spec.round_to_tick(five.close + slip * d)',
     '        fill = self.spec.round_to_tick(five.close - slip * d)',
     S_IB),
    ('ib__the_ambiguous_bar_resolves_to_the_target', IBE,
     '        if hit_stop:\n            slip = self.spec.execution.stop_ticks * tick',
     '        if hit_stop and not hit_target:\n            slip = self.spec.execution.stop_ticks * tick',
     S_IB),
    ('ib__killswitch_moved_to_minus_801', IBS,
     '    daily_loss_killswitch: float = -800.0',
     '    daily_loss_killswitch: float = -801.0',
     S_IB_GOV),
    ('ib__killswitch_reads_the_close_not_the_adverse_extreme', IBE,
     '        adverse = five.low if d > 0 else five.high',
     '        adverse = five.close',
     S_IB),
    ('ib__profit_cap_moved_to_1201', IBS,
     '    daily_profit_cap: float = 1200.0',
     '    daily_profit_cap: float = 1201.0',
     S_IB_GOV),
    ('ib__the_profit_cap_boundary_becomes_strict', IBG,
     '        return self.realized >= self.spec.daily_profit_cap',
     '        return self.realized > self.spec.daily_profit_cap',
     S_IB_GOV),
    ('ib__trade_cap_raised_to_three', IBS,
     '    max_trades_per_day: int = 2',
     '    max_trades_per_day: int = 3',
     S_IB),
    ('ib__time_stop_extended', IBS,
     '    time_stop_minutes: int = 60',
     '    time_stop_minutes: int = 75',
     S_IB),
    ('ib__the_time_stop_never_fires', IBE,
     '        if when >= p.deadline:',
     '        if False:',
     S_IB),
]
